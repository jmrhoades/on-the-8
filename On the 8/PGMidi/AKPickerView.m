...
- (void)initialize
{
	...
	_maskLayer.frame = self.collectionView.bounds;
	_maskLayer.colors = @[(id)[[UIColor clearColor] CGColor],
						 (id)[[UIColor blackColor] CGColor],
						 (id)[[UIColor blackColor] CGColor],
						 (id)[[UIColor clearColor] CGColor],];
	_maskLayer.locations = @[@0.0, @0.33, @0.66, @1.0];
	_maskLayer.startPoint = CGPointMake(0.0, 0.0);
	_maskLayer.endPoint = CGPointMake(1.0, 0.0);
	self.collectionView.layer.mask = _maskLayer;
}
...
- (void)layoutSubviews
{
    ...
    if (_isLeftAligned) {
        _maskLayer.locations = @[@0.0, @0.05, @0.9, @1.0];
    } else {
...
- (void)didEndScrolling
{
	switch (self.pickerViewStyle) {
		case AKPickerViewStyleFlat: {
            CGPoint center;
            center = [self convertPoint:self.collectionView.center toView:self.collectionView];
...
{
    NSLog(@"AKPicker didSelectItemAtIndexPath");
    [self selectItem:indexPath.item animated:YES];
}
...
- (void)setSelected:(BOOL)selected
{
    ...
    UIColor *c = selected ? StyleKit.yellow1 : StyleKit.gray1;
    //[UIView transitionWithView:self.label duration:0.33 options:UIViewAnimationOptionTransitionCrossDissolve animations:^{
