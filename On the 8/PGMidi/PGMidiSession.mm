...
- (void) midi:(PGMidi*)midi sourceAdded:(PGMidiSource *)source {
    //NSLog(@"PGMidiSessionDelegate: sourceAdded");
...
{
    //NSLog(@"PGMidiSessionDelegate: midiReceived");
    ...
    for (int i = 0; i < packetList->numPackets; ++i)
    {
        ...
        //NSLog(@"PGMidiSessionDelegate: status %d", statusByte);
			...
			if (playing)
			{
                ...
                for (NSUInteger j = 0; j < quantizedBlockQueue.count; j++)
                {
                    ...
                    if (currentNumTicks % interval == 0 && qb.extraBars <= 0)
                    {
                        ...
                        if(qb.repeat == NO)
                        {
                            //if (quantizedBlockQueue.count > j) {
                                //NSLog(@"IS IT HERE?");
                ...
                [delegate tickUpdate:currentNumTicks];
			}
        ...
        if (status == VVMIDIControlChangeVal)
        {
            dispatch_async(dispatch_get_main_queue(), ^
                           {
